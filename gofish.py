...
def drawCard(name, deck, hand):
  ...
  if len(deck) > 0: # protect against empty deck
    ...
    cardRank = newCard[0]
    cardSuit = newCard[1]
    # print(name, 'draws', cardRank, 'of', cardSuit, 'hand length', len(hand))
...
decks = []
# use int instead of string for convenience
for spades in range(13):
    decks.append((str(spades), 'spades'))
for hearts in range(13):
    decks.append((str(hearts), 'hearts'))
for diamonds in range(13):
    decks.append((str(diamonds), 'diamonds'))
for clubs in range(13):
    decks.append((str(clubs), 'clubs'))

playerAhand = dict()
playerBhand = dict()

for turn in range(26):
    drawCard('playerA', decks, playerAhand)
    # print('playerA', playerAhand)
    drawCard('playerB', decks, playerBhand)
    # print('playerB', playerBhand)
